...
def input_transform():
    return transforms.Compose([
        ...
        transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[
                             0.229, 0.224, 0.225]),
        # transforms.Normalize(mean=[0.574, 0.545, 0.495], std=[
        #                      0.083, 0.081, 0.081]),
    ])


def get_whole_training_set(opt, onlyDB=False, forCluster=False):
    return WholeDatasetFromStruct(opt, join(opt.structDir, 'train.mat'), opt.imgDir, input_transform=input_transform(), onlyDB=onlyDB, forCluster=forCluster)


def get_whole_val_set(opt):
    return WholeDatasetFromStruct(opt, join(opt.structDir, 'val.mat'), opt.imgDir, input_transform=input_transform())


def get_whole_test_set(opt):
    return WholeDatasetFromStruct(opt, join(opt.structDir, 'test.mat'), opt.imgDir, input_transform=input_transform())


def get_training_query_set(opt, margin=0.1):
    return QueryDatasetFromStruct(opt, join(opt.structDir, 'train.mat'), opt.imgDir, input_transform=input_transform(), margin=margin)


def get_val_query_set(opt, margin=0.1):
    return QueryDatasetFromStruct(opt, join(opt.structDir, 'val.mat'), opt.imgDir, input_transform=input_transform(), margin=margin)
